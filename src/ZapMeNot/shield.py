...
class SemiInfiniteXSlab(Shield):
    ...
    def getCrossingLength(self, ray):
        ...
        firstLength = self.LinePlaneCollision(
            planeNormal, planePoint, rayOrigin, rayUnitVector)
        if firstLength is None:
            # ray is parallel to plane
            return 0
        # get length to second crossing point
        planePoint = np.array([self.xEnd, 0, 0])
        secondLength = self.LinePlaneCollision(
            planeNormal, planePoint, rayOrigin, rayUnitVector)
        if secondLength is None:
            # ray is parallel to plane
            return 0
...
class Box(Shield):
    ...
    def intersectAABox(self, ray):
        ...
        results = []
        bounds = [self.boxCenter - (self.boxDimensions/2),
                  self.boxCenter + (self.boxDimensions/2)]
...
class CappedCylinder(Shield):
    ...
    def intersect(self, ray):
        ...
        for diskCenter in [self.origin, self.end]:
            t = self.LinePlaneCollision(
                self.dir, diskCenter, ray.origin, ray.dir)
